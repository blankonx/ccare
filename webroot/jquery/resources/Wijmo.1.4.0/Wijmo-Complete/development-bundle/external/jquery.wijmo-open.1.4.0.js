...
(function ($) {
	...
	$.widget("wijmo.wijsplitter", {
		...
		_updateExpanderCss: function () {
			...
			expander.removeClass(cornerCssPrefix + collapsedExpCorner1Css +
		...
		_bindEvents: function () {
			...
			.bind("mouseover." + widgetName, function (e) {
				if (o.disabled) {
					return;
				}
				bar.addClass(stateHoverCss);
...
(function ($) {
	...
	$.widget("wijmo.wijdialog", $.ui.dialog, {
		...
		_create: function () {
			...
			self.originalPosition = self.options.position;
		...
		_initWijWindow: function () {
			...
			.bind("focusout", function (event) {
				var el = event.target, container = $(this);
				if (!isIn) {
		...
		reset: function () {
			...
			self.normalInnerMinWidth = self.normalInnerMinHeight
			= undefined;
...
})(jQuery);/*globals window document jQuery */
/*
*
* Wijmo Library 1.4.0
* http://wijmo.com/
	 ...
	 $.widget("wijmo.wijtabs", {
		...
		panelCorner = this._getAlignment(false);
		 ...
		 load: function (index) {
			 ...
			 if (false === this._trigger('beforeShow', null, self._ui(self.anchors[index], self.panels[index]))) {
