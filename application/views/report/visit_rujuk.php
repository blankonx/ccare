...
$(document).ready(function() {
	...
	$.ajax({
		url: 'visit_rujuk/get_region_by_village_id/' + <?php echo $profile['village_id']?>,
		dataType: 'json',
		success:function(data) {
...
function get_sub_district(val) {
	$.get('visit_rujuk/get_sub_district/' + val, function(data) {$('#sub_district_id').html(data);});
	$.get('visit_rujuk/get_village/0', function(data) {$('#village_id').html(data);});
}

function get_village(val) {
	$.get('visit_rujuk/get_village/' + val, function(data) {$('#village_id').html(data);});
}
</script>
<div class="ui-dialog" style="width:100%;margin-right:5px;height:auto;float:left;">
	...
	<div class="ui-dialog-content" id="dialogContent">
		<form method="POST" name="frmReport" id="frmReport" action="<?php echo site_url('report/visit_rujuk/process_form')?>">
