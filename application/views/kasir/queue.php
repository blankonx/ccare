...
$(document).ready(function() {
	...
	function getList() {
		$('#divSearchResult').load("<?php echo site_url('kasir/queue_list/result/_');?>" + "/" + $('#current_page').val(),prepareList);return false;
	}
//ajaxing search form
	...
	function prepareList() {
		...
		$('.checkup_link').parent().parent().dblclick(function() {
            //$('#panel_checkup').slideDown('fast');
            var xurl = '../kasir/checkup/result/' + $(this).attr('id') + '/' + checkup_cnt + '/';
            $('#panel_checkup_content').load(xurl);
            $('#panel_checkup').show();
            checkup_cnt++;
            return false;
...
<div class="smallSearchContainer" id="div_search" style="display:none;right:16px;top:25px">
	...
	<form method="POST" name="frmSearch" id="frmSearch" action="<?php echo site_url('kasir/queue_list');?>">
		<input type="hidden" name="xcurrent_page" id="xcurrent_page" value="" />
		<input type="text" name="search_name" id="search_name" value="" size="25" onkeypress="focusNext('visit_date_start', 'visit_date_start', this, event)" />
		
		<div style="text-align:right;">
        <input type="text" name="visit_date_start" class="date-pick" id="visit_date_start" maxlength="10" value="<?=date('d/m/Y')?>" size="12" onkeyup="autoSlashTanggal(this, event)" onkeypress="focusNext('visit_date_end', 'search_name', this, event)" />&minus;<input type="text" name="visit_date_end" id="visit_date_end" maxlength="10" value="<?=date('d/m/Y')?>" size="12" onkeyup="autoSlashTanggal(this, event)" onkeypress="focusNext('search_name', 'visit_date_start', this, event)" class="date-pick" />
...
<div class="ui-dialog" style="width:100%;margin-right:5px;height:auto;float:left;">
	<div style="position: relative;" class="ui-dialog-container">
		...
		<div class="ui-dialog-content">
			...
			<form method="POST" name="frmList" id="frmList" action="<?php echo site_url('kasir/queue/delete_list');?>">
			...
			<ul class="legend">
				...
				<li><img alt="Ubah Data Pemeriksaan Pasien" src="<?php echo base_url();?>webroot/media/images/check.png"/>&nbsp;Ubah Tagihan Pasien</li>
				<li><img alt="Cetak Pemeriksaan" src="<?php echo base_url();?>webroot/media/images/print16.png"/>&nbsp;Cetak Kwitansi</li>
			</ul>
		</div><!-- 
