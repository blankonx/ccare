...
//get List Data
	function getList() {
		$('#divSearchResult').load("<?php echo site_url('admdata/religion_list/result/_');?>" + "/" + $('#current_page').val(),prepareList);return false;
	}
...
//ajaxing add data
	...
	$('#reset').click( function() {getList();$('#preview_table').slideUp('fast');$('#frmInput').clearForm();$('#saved_id').val('');$('#id').focus();});
	$('#frmSearch').resetForm();
	$('#frmSearch').submit();
    $('#id').focus();
});
...
<div class="smallSearchContainer" id="div_search" style="display:none;">
	...
	<form method="POST" name="frmSearch" id="frmSearch" action="<?php echo site_url('admdata/religion_list');?>">
...
<div class="ui-dialog" style="width:100%;margin-right:5px;height:auto;float:left;">
	<div style="position: relative;" class="ui-dialog-container">
		<div class="ui-dialog-titlebar">Data Pelayanan</div>
		...
		<form method="POST" name="frmInput" id="frmInput" style="display:none" action="<?php echo site_url('admdata/religion/process_form')?>">
		...
		<table cellpadding="0" cellspacing="0" border="0" class="tblInput">
			<tr>
				...
				<td>
					<input type="text" name="id" id="id" size="3" class="required" />
				</td>
			</tr>
			<tr>
				<td>Agama</td>
			...
			<form method="POST" name="frmList" id="frmList" action="<?php echo site_url('admdata/religion/delete_list');?>">
