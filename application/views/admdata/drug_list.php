	...
	<?php else :?>
		...
		<?php if($list[$i]['category'] != $list[$i-1]['category']) :?>
			<tr class="notSelected">
				<td colspan="5" style="background:#F9F9F9;"><b><?php echo $list[$i]['category'];?></b></td>
			</tr>
		<?php endif;?>
		<tr class="notSelected">
			<td>
			...
			<td ondblclick="get_data_by_id('<?php echo $list[$i]['id'];?>')"><?php echo $list[$i]['unit'];?></td>
			<td ondblclick="get_data_by_id('<?php echo $list[$i]['id'];?>')"><?php echo $list[$i]['supplier'];?></td>
		</tr>
		<?php endfor;?>
	<?php endif;?>
	</tbody>
