...
Class Payment extends Controller {
    ...
    function __construct() {
        ...
        $this->load->model('admdata/Payment_Model');
        //$this->js[] = "jquery/js/jquery.jeditable.js";
		$this->title = "Data Jenis Pasien";
    }
    ...
    function get_data_by_id() {
        $ret = $this->Payment_Model->GetDataById();
		echo json_encode($ret);
    }
    
    function delete_list() {
        $ret['command'] = "deleting data";
        if($this->Payment_Model->DoDeleteData()) {
    ...
    function process_form() {
        ...
        if($this->input->post('saved_id')) {
			...
			if($this->Payment_Model->DoUpdateData()) {
			...
			$last_id = $this->Payment_Model->DoAddData();
			if($last_id > 0) {
...
//VIEW//
    function _view($data = array()) {
        ...
        $this->load->view('admdata/payment', $data);
