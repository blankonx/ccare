...
Class Supplier extends Controller {
    ...
    function __construct() {
        ...
        $this->load->model('admdata/Supplier_Model');
    ...
    function get_data_by_id() {
        $ret = $this->Supplier_Model->GetDataById();
		echo json_encode($ret);
    }
    
    function delete_list() {
        $ret['command'] = "deleting data";
        if($this->Supplier_Model->DoDeleteData()) {
    ...
    function process_form() {
        ...
        if($this->input->post('id')) {
			...
			if($this->Supplier_Model->DoUpdateData()) {
			...
			$last_id = $this->Supplier_Model->DoAddData();
			if($last_id > 0) {
...
//VIEW//
    function _view($data = array()) {
        ...
        $this->load->view('admdata/supplier', $data);
