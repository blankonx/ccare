...
Class Religion extends Controller {
    ...
    function __construct() {
        ...
        $this->load->model('admdata/Religion_Model');
        //$this->js[] = "jquery/js/jquery.jeditable.js";
		$this->title = "Data Religion";
    }
    ...
    function get_data_by_id() {
        $ret = $this->Religion_Model->GetDataById();
		echo json_encode($ret);
    }
    
    function delete_list() {
        $ret['command'] = "deleting data";
        if($this->Religion_Model->DoDeleteData()) {
    ...
    function process_form() {
        ...
        if($this->input->post('saved_id')) {
			...
			if($this->Religion_Model->DoUpdateData()) {
			...
			$last_id = $this->Religion_Model->DoAddData();
			if($last_id > 0) {
...
//VIEW//
    function _view($data = array()) {
        ...
        $this->load->view('admdata/religion', $data);
