...
Class Payment_List extends Controller {
    ...
    function __construct() {
        ...
        $this->load->model('admdata/Payment_List_Model');
    }
    ...
    function result($str_search='', $start=0) {
		...
		$data['list'] = $this->Payment_List_Model->GetData($str_search, $start, $this->offset);
		$data['total'] = $this->Payment_List_Model->getCount();
		$data['start'] = $start;

		//print_r($data);
		$paging = array();
		$paging['base_url'] = base_url() . 'admdata/payment_list/result/' . $str_search . '/';
...
//VIEW//
    function _view($data = array()) {
        $this->load->view('admdata/payment_list', $data);
    }
}
