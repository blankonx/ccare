...
Class Icd_List extends Controller {
    ...
    function __construct() {
        ...
        $this->load->model('admdata/Icd_List_Model');
    }
    ...
    function result($str_search='', $start=0) {
		...
		$data['list'] = $this->Icd_List_Model->GetData($str_search, $start, $this->offset);
		$data['total'] = $this->Icd_List_Model->getCount();
		$data['start'] = $start;

		//print_r($data);
		$paging = array();
		$paging['base_url'] = base_url() . 'admdata/icd_list/result/' . $str_search . '/';
...
//VIEW//
    function _view($data = array()) {
        $this->load->view('admdata/icd_list', $data);
    }
}
