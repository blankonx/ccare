...
Class Religion_List extends Controller {
    ...
    function __construct() {
        parent::Controller();
        //if(!$this->session->religiondata('id')) redirect('login');
        $this->load->model('admdata/Religion_List_Model');
    }
    ...
    function result($str_search='', $start=0) {
		...
		$data['list'] = $this->Religion_List_Model->GetData($str_search, $start, $this->offset);
		$data['total'] = $this->Religion_List_Model->getCount();
		$data['start'] = $start;

		//print_r($data);
		$paging = array();
		$paging['base_url'] = base_url() . 'admdata/religion_list/result/' . $str_search . '/';
...
//VIEW//
    function _view($data = array()) {
        $this->load->view('admdata/religion_list', $data);
    }
}
