<?php
Class Lap_Rujukan extends Controller {
	var $title = 'Laporan Rujukan';
	var $report_title = 'Laporan Rujukan';

	function __construct() {
		parent::Controller();
		if(!$this->session->userdata('id')) redirect('login');
		$this->load->model('report/Lap_Rujukan_Model');
	...
	function index($ret="") {
		...
		$data['combo_payment_type'] = $this->Lap_Rujukan_Model->GetComboPaymentType();
		$this->_view($data);	      	
	}
	...
	function process_form() {
		 ...
		 * */
        $profile = $this->xProfile_Model->GetProfile();
        $visit = $this->Lap_Rujukan_Model->BackupVisits($periode,$profile);
		...
		if($this->input->post('payment_type_id') != '') {
			$data['report_title'] .= ' Jenis Pasien ' . $this->Lap_Rujukan_Model->GetDataPaymentTypeName($this->input->post('payment_type_id'));
		}
		
        for($i=0;$i<sizeof($visit);$i++) {
			//kode_klinik, paramedic, lanjutan, nik,nama_pasien, tgl_lahir, umur,jenis kelamin,tgl_kunjungan, nama_status_jaminan, kode icdx, deskripsi tindakanmedis,ttotal biaya, 

            $usia = getAge($visit[$i]['birth_date'], $visit[$i]['visit_date']);
			$vis .= $visit[$i]['kode_klinik'];
			$vis .= $visit[$i]['paramedic'];
			$vis .= $visit[$i]['continue_to']. "|" . $visit[$i]['specialis'];
            $vis .= $visit[$i]['nik'] . ";";
			$vis .= $visit[$i]['name'].';'; 
        ...
        if($this->input->post('metode') == "service") {
            ...
            redirect('report/lap_rujukan');
        } else {
				
				$data = array(
                'rujukan_'. $periode . "-" . underscore($profile['name']).'.csv' => $vis
            );
			$this->load->library('zip');
			$this->zip->add_data($data);
			$this->zip->archive('.'. dirname($_SERVER['SCRIPT_FILENAME']) .'/webroot/lap_rujukan/rujukan_'. $periode . "_" . underscore($profile['name']).'.zip');			
			$datazip = $this->zip->download('rujukan_'. $periode . "_" . underscore($profile['name']).'.zip'); 
        }
	}
	...
	function _view($data = array()) {
		...
		$this->load->view('report/lap_rujukan', $data);
