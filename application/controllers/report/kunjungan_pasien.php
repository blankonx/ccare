...
Class Kunjungan_Pasien extends Controller {
	var $title = '';
	var $report_title = 'Kunjungan Pasien';

	function __construct() {
		parent::Controller();
		if(!$this->session->userdata('id')) redirect('login');
		$this->load->model('report/Kunjungan_Pasien_Model');
	...
	function index($data="") {
        ...
        $data['combo_district'] = $this->Kunjungan_Pasien_Model->GetComboDistrict();
		$data['combo_clinic'] = $this->Kunjungan_Pasien_Model->GetComboClinics(); 	
		$data['combo_payment_type'] = $this->Kunjungan_Pasien_Model->GetComboPaymentType();
		$this->_view($data);	      	
	}
	...
	function process_form() {
		...
		if($this->input->post('clinic_id') != '') {
			$data['report_title'] .= $this->lang->line('label_clinic') . ' ' . $this->Kunjungan_Pasien_Model->GetDataClinicName($this->input->post('clinic_id'));
		}
        
		if($this->input->post('payment_type_id') != '') {
			$data['report_title'] .= ' Jenis Pasien ' . $this->Kunjungan_Pasien_Model->GetDataPaymentTypeName($this->input->post('payment_type_id'));
		}
		
		$data['report_sub_title_wilayah'] = "";
		if($this->input->post('general_spesifik') == 'spesifik') {
			$data['report_sub_title_wilayah'] = $this->Kunjungan_Pasien_Model->GetRegionName($this->input->post('village_id'), $this->input->post('sub_district_id'), $this->input->post('district_id'));
		} else {
			switch($region) {
				case "in" :
		...
		$data['list'] = $this->Kunjungan_Pasien_Model->GetDataVisitSensus();
        $this->load->view('report/kunjungan_pasien_list', $data);
	}
    ...
    function excel($title="") {
		...
		if($report_param['clinic_id'] != '') {
			$data['report_title'] .= $this->lang->line('label_clinic') . ' ' . $this->Kunjungan_Pasien_Model->GetDataClinicName($report_param['clinic_id']);
		}
        
		if($report_param['payment_type_id'] != '') {
			$data['report_title'] .= ' Jenis Pasien ' . $this->Kunjungan_Pasien_Model->GetDataPaymentTypeName($report_param['payment_type_id']);
		}
		
		$data['report_sub_title_wilayah'] = "";
		if($report_param['general_spesifik'] == 'spesifik') {
			$data['report_sub_title_wilayah'] = $this->Kunjungan_Pasien_Model->GetRegionName($report_param['village_id'], $report_param['sub_district_id'], $report_param['district_id']);
		} else {
        ...
        header('Content-Disposition: inline; filename=Sensus_Rawat_Jalan_' . underscore($data["report_sub_title"]) . '.xls');
        header('Expires: 0');
        header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
        header('Pragma: public');
		//print_r($report_param);
		$data['list'] = $this->Kunjungan_Pasien_Model->GetDataVisitSensusPrint($report_param);
		//$this->load->view('_header_print_full_landscape', $data);
		$this->load->view('report/kunjungan_pasien_excel', $data);
	...
	function printout($title="") {
		...
		$data['list'] = $this->Kunjungan_Pasien_Model->GetDataVisitSensusPrint($report_param);
		$this->load->view('_header_print_full_landscape', $data);
		$this->load->view('report/kunjungan_pasien_print', $data);
		$this->load->view('_footer_print_full');
	}
	...
	function _view($data = array()) {
		...
		$this->load->view('report/kunjungan_pasien', $data);
		$this->load->view('_footer');
	}
	
	function get_region_by_village_id($villageId) {
		//kabupaten
        $district = $this->Kunjungan_Pasien_Model->GetDistrictByVillageId($villageId);
		$ret['district_id'] = $district['id'];

        $sub_district = $this->Kunjungan_Pasien_Model->GetSubDistrictByVillageId($villageId);
        $arr_sub_district = $this->Kunjungan_Pasien_Model->GetComboSubDistrictByVillageId($villageId);
        ...
        $arr_village = $this->Kunjungan_Pasien_Model->GetComboVillageById($villageId);
	...
	function get_sub_district($districtId='1') {
        $data = $this->Kunjungan_Pasien_Model->GetComboSubDistrict($districtId);
	...
	function get_village($subDistrictId='1') {
        $data = $this->Kunjungan_Pasien_Model->GetComboVillage($subDistrictId);
		$ret = '<option value="">--- '.$this->lang->line('form_all').' ---</option>';
		for($i=0;$i<sizeof($data);$i++) {
			$ret .= '<option value="'.$data[$i]['id'].'">'.$data[$i]['name'].'</option>';
		}
