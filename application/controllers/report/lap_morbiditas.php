<?php
Class Lap_Morbiditas extends Controller {
	var $title = 'Laporan Morbiditas';
	var $report_title = 'Laporan Morbiditas';

	function __construct() {
		parent::Controller();
		if(!$this->session->userdata('id')) redirect('login');
		$this->load->model('report/Lap_Morbiditas_Model');
	...
	function index($ret="") {
		...
		$data['combo_payment_type'] = $this->Lap_Morbiditas_Model->GetComboPaymentType();
		$this->_view($data);	      	
	}
	...
	function process_form() {
		 ...
		 * */
        $profile = $this->xProfile_Model->GetProfile();
        $visit = $this->Lap_Morbiditas_Model->BackupVisits($periode,$profile);
		...
		if($this->input->post('payment_type_id') != '') {
			$data['report_title'] .= ' Jenis Pasien ' . $this->Lap_Morbiditas_Model->GetDataPaymentTypeName($this->input->post('payment_type_id'));
		}
        ...
        for($i=0;$i<sizeof($visit);$i++) {
            ...
            $vis .= $usia['day'] . ';';
			$vis .= $visit[$i]['sex'] . ';';
			$vis .= $visit[$i]['code_desa'] . ';';
			$vis .= $visit[$i]['nama_desa'] . ';';
			$vis .= $visit[$i]['visit_date'] . ';';
			$vis .= $visit[$i]['kode_jaminan'] . ';';
			$vis .= $visit[$i]['diagnose'] . ';';
        ...
        if($this->input->post('metode') == "service") {
            ...
            redirect('report/lap_morbiditas');
        } else {
				
				$data = array(
                'morbiditas_'. $periode . "_" . underscore($profile['name']).'.csv' => $vis
            );
			$this->load->library('zip');
			$this->zip->add_data($data);
			$this->zip->archive('.'. dirname($_SERVER['SCRIPT_FILENAME']) .'/webroot/lap_morbiditas/morbiditas_'. $periode . "_" . underscore($profile['name']).'.zip');			
			$datazip = $this->zip->download('morbiditas_'. $periode . "_" . underscore($profile['name']).'.zip'); 
        }
	}
	...
	function _view($data = array()) {
		...
		$this->load->view('report/lap_morbiditas', $data);
