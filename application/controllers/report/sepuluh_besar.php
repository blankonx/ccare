...
Class Sepuluh_Besar extends Controller {
	var $title = '';
	...
	function __construct() {
		...
		$this->load->model('report/Sepuluh_Besar_Model');
		//$this->load->model('report/Sepuluh_Besar_Model');
	...
	function index($data="") {
		...
		$data['combo_payment_type'] = $this->Sepuluh_Besar_Model->GetComboPaymentType();
        $data['combo_kelompok_penyakit'] = $this->Sepuluh_Besar_Model->GetComboKelompokPenyakit();
		$this->_view($data);	      	
	}
	...
	function process_form() {
		...
		$data['report_title'] .= $this->Sepuluh_Besar_Model->GetNamaKelompokPenyakit($this->input->post('icd_group_id'));
		
		
		if($this->input->post('payment_type_id') != '') {
			$data['report_title'] .= ' Jenis Pasien ' . $this->Sepuluh_Besar_Model->GetDataPaymentTypeName($this->input->post('payment_type_id'));
		}
		if($this->input->post('kelompok_umur') != '') {
			$data['report_title'] .= ' Umur ' . $this->input->post('kelompok_umur');
		}
		if($this->input->post('sex') != '') {
			$data['report_title'] .= ' (' . $this->input->post('sex') . ')';
		}
		$data['report_sub_title_wilayah'] = "";
		if($this->input->post('general_spesifik') == 'spesifik') {
			$data['report_sub_title_wilayah'] = $this->Sepuluh_Besar_Model->GetRegionName($this->input->post('village_id'), $this->input->post('sub_district_id'), $this->input->post('district_id'));
		} else {
			switch($region) {
				case "in" :
		...
		/*CHART BEGIN*/
		$param = array("Column3D", 700, 300, 'Sepuluh_BesarChart');
		...
		$data['chart'] = $this->Sepuluh_Besar_Model->GetDataSepuluh_BesarForChart();
		...
		/*CHART END*/
        $this->load->view('report/sepuluh_besar_list', $data);
	}
	...
	function printout($title="") {
		...
		$this->load->view('report/sepuluh_besar_print', $data);
	...
	function _view($data = array()) {
		...
		$this->load->view('report/sepuluh_besar', $data);
