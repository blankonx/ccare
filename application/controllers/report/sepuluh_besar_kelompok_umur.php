...
Class Sepuluh_Besar_Kelompok_Umur extends Controller {
	var $title = '';
	var $report_title = '10 Besar Penyakit';

	function __construct() {
		parent::Controller();
		if(!$this->session->userdata('id')) redirect('login');
		//$this->load->model('admission/Indoor_Model');
		$this->load->model('report/Sepuluh_Besar_Kelompok_Umur_Model');
		//$this->load->model('report/Sepuluh_Besar_Kelompok_Umur_Model');
	...
	function index($data="") {
        ...
        $data['combo_district'] = $this->Sepuluh_Besar_Kelompok_Umur_Model->GetComboDistrict();
		$data['combo_clinic'] = $this->Sepuluh_Besar_Kelompok_Umur_Model->GetComboClinics();
		$data['combo_payment_type'] = $this->Sepuluh_Besar_Kelompok_Umur_Model->GetComboPaymentType();
		$this->_view($data);	      	
	}
	...
	function process_form() {
		...
		switch($tampilkan) {
			case "5" :
				$data['report_title'] = "Statistik 5 Besar Penyakit";
			break;
			case "10" :
				$data['report_title'] = "Statistik 10 Besar Penyakit";
			break;
			case "20" :
				$data['report_title'] = "Statistik 20 Besar Penyakit";
			break;
			case "50" :
				$data['report_title'] = "Statistik 50 Besar Penyakit";
			break;
			default :
				$data['report_title'] = "Statistik Penyakit";
			break;
		} 
		
		if($this->input->post('clinic_id') != '') {
			$data['report_title'] .= ' ' . $this->lang->line('label_clinic') . ' ' . $this->Sepuluh_Besar_Kelompok_Umur_Model->GetDataClinicName($this->input->post('clinic_id'));
		}
		if($this->input->post('payment_type_id') != '') {
			$data['report_title'] .= ' Jenis Pasien ' . $this->Sepuluh_Besar_Kelompok_Umur_Model->GetDataPaymentTypeName($this->input->post('payment_type_id'));
		}
		if($this->input->post('kelompok_umur') != '') {
			$data['report_title'] .= ' Umur ' . $this->input->post('kelompok_umur');
		}
		$data['report_sub_title_wilayah'] = "";
		if($this->input->post('general_spesifik') == 'spesifik') {
			$data['report_sub_title_wilayah'] = $this->Sepuluh_Besar_Kelompok_Umur_Model->GetRegionName($this->input->post('village_id'), $this->input->post('sub_district_id'), $this->input->post('district_id'));
		} else {
			switch($region) {
				case "in" :
		...
		/*CHART BEGIN*/
		$param = array("Column3D", 700, 300, 'Sepuluh_Besar_Kelompok_UmurChart');
		$this->load->library('chart', $param);
		$data['chart'] = $this->Sepuluh_Besar_Kelompok_Umur_Model->GetDataSepuluh_Besar_Kelompok_UmurForChart();
		...
		/*CHART END*/
        $this->load->view('report/sepuluh_besar_kelompok_umur_list', $data);
	}
	...
	function printout($title="") {
		...
		$this->load->view('report/sepuluh_besar_kelompok_umur_print', $data);
	...
	function _view($data = array()) {
		...
		$this->load->view('report/sepuluh_besar_kelompok_umur', $data);
	...
	function get_region_by_village_id($villageId) {
		//kabupaten
        $district = $this->Sepuluh_Besar_Kelompok_Umur_Model->GetDistrictByVillageId($villageId);
		$ret['district_id'] = $district['id'];

        $sub_district = $this->Sepuluh_Besar_Kelompok_Umur_Model->GetSubDistrictByVillageId($villageId);
        $arr_sub_district = $this->Sepuluh_Besar_Kelompok_Umur_Model->GetComboSubDistrictByVillageId($villageId);
        ...
        $arr_village = $this->Sepuluh_Besar_Kelompok_Umur_Model->GetComboVillageById($villageId);
	...
	function get_sub_district($districtId='1') {
        $data = $this->Sepuluh_Besar_Kelompok_Umur_Model->GetComboSubDistrict($districtId);
	...
	function get_village($subDistrictId='1') {
        $data = $this->Sepuluh_Besar_Kelompok_Umur_Model->GetComboVillage($subDistrictId);
		$ret = '<option value="">--- '.$this->lang->line('form_all').' ---</option>';
		for($i=0;$i<sizeof($data);$i++) {
			$ret .= '<option value="'.$data[$i]['id'].'">'.$data[$i]['name'].'</option>';
		}
