...
Class Patient_Job extends Controller {
	var $title = '';
	var $report_title = 'Statistik Pasien Berdasar Pekerjaan';

	function __construct() {
		parent::Controller();
		if(!$this->session->userdata('id')) redirect('login');
		//$this->load->model('admission/Indoor_Model');
		$this->load->model('report/Patient_Job_Model');
		//$this->load->model('report/Patient_Job_Model');
	...
	function process_form() {
		...
		/*CHART BEGIN*/
		$param = array("Pie3D", 700, 300, 'Patient_JobChart');
		$this->load->library('chart', $param);
		$data['chart'] = $this->Patient_Job_Model->GetDataPatient_JobForChart();
		...
		/*CHART END*/
        $this->load->view('report/patient_job_list', $data);
	}
	...
	function printout($title="") {
		...
		$this->load->view('report/patient_job_print', $data);
	...
	function _view($data = array()) {
		...
		$this->load->view('report/patient_job', $data);
