...
Class Patient_Sex extends Controller {
	var $title = '';
	var $report_title = 'Statistik Pasien Berdasar Jenis Kelamin';

	function __construct() {
		parent::Controller();
		if(!$this->session->userdata('id')) redirect('login');
		//$this->load->model('admission/Indoor_Model');
		$this->load->model('report/Patient_Sex_Model');
		//$this->load->model('report/Patient_Sex_Model');
	...
	function index($data="") {
        ...
        $data['combo_district'] = $this->Patient_Sex_Model->GetComboDistrict();
		$this->_view($data);	      	
	}
	...
	function process_form() {
		...
		if($this->input->post('general_spesifik') == 'spesifik') {
			$data['report_sub_title_wilayah'] = $this->Patient_Sex_Model->GetRegionName($this->input->post('village_id'), $this->input->post('sub_district_id'), $this->input->post('district_id'));
		} else {
			switch($region) {
				case "in" :
		...
		/*CHART BEGIN*/
		$param = array("Pie3D", 700, 300, 'Patient_SexChart');
		$this->load->library('chart', $param);
		$data['chart'] = $this->Patient_Sex_Model->GetDataPatient_SexForChart();
		...
		/*CHART END*/
        $this->load->view('report/patient_sex_list', $data);
	}
	...
	function printout($title="") {
		...
		$this->load->view('report/patient_sex_print', $data);
	...
	function _view($data = array()) {
		...
		$this->load->view('report/patient_sex', $data);
	...
	function get_region_by_village_id($villageId) {
		//kabupaten
        $district = $this->Patient_Sex_Model->GetDistrictByVillageId($villageId);
		$ret['district_id'] = $district['id'];

        $sub_district = $this->Patient_Sex_Model->GetSubDistrictByVillageId($villageId);
        $arr_sub_district = $this->Patient_Sex_Model->GetComboSubDistrictByVillageId($villageId);
        ...
        $arr_village = $this->Patient_Sex_Model->GetComboVillageById($villageId);
	...
	function get_sub_district($districtId='1') {
        $data = $this->Patient_Sex_Model->GetComboSubDistrict($districtId);
	...
	function get_village($subDistrictId='1') {
        $data = $this->Patient_Sex_Model->GetComboVillage($subDistrictId);
		$ret = '<option value="">--- '.$this->lang->line('form_all').' ---</option>';
		for($i=0;$i<sizeof($data);$i++) {
			$ret .= '<option value="'.$data[$i]['id'].'">'.$data[$i]['name'].'</option>';
		}
