...
Class Sepuluh_Besar_Anak_Dewasa extends Controller {
	var $title = '';
	var $report_title = '10 Besar Penyakit Anak-Dewasa';

	function __construct() {
		parent::Controller();
		if(!$this->session->userdata('id')) redirect('login');
		//$this->load->model('admission/Indoor_Model');
		$this->load->model('report/Sepuluh_Besar_Anak_Dewasa_Model');
		//$this->load->model('report/Sepuluh_Besar_Anak_Dewasa_Model');
	...
	function index($data="") {
		...
		$data['combo_payment_type'] = $this->Sepuluh_Besar_Anak_Dewasa_Model->GetComboPaymentType();
		$this->_view($data);	      	
	}
	...
	function process_form() {
		...
		switch($tampilkan) {
			case "5" :
				$data['report_title'] = "Statistik 5 Besar Penyakit Anak-Dewasa";
			break;
			case "10" :
				$data['report_title'] = "Statistik 10 Besar Penyakit Anak-Dewasa";
			break;
			case "20" :
				$data['report_title'] = "Statistik 20 Besar Penyakit Anak-Dewasa";
			break;
			case "50" :
				$data['report_title'] = "Statistik 50 Besar Penyakit Anak-Dewasa";
			break;
			default :
				$data['report_title'] = "Statistik Penyakit Anak-Dewasa";
			break;
		} 
		
		
		if($this->input->post('payment_type_id') != '') {
			$data['report_title'] .= ' Jenis Pasien ' . $this->Sepuluh_Besar_Anak_Dewasa_Model->GetDataPaymentTypeName($this->input->post('payment_type_id'));
		}
		$data['report_sub_title_wilayah'] = "";
		if($this->input->post('general_spesifik') == 'spesifik') {
			$data['report_sub_title_wilayah'] = $this->Sepuluh_Besar_Anak_Dewasa_Model->GetRegionName($this->input->post('village_id'), $this->input->post('sub_district_id'), $this->input->post('district_id'));
		} else {
			switch($region) {
				case "in" :
		...
		/*CHART BEGIN*/
		$param = array("MSColumn3D", 700, 300, 'Sepuluh_Besar_Anak_DewasaChart');
		$this->load->library('chart', $param);
		$this->chart->setChartParams('showValues=1;formatNumberScale=0;showYAxisValues=1;formatNumber=1;adjustDiv=0;xAxisName=ICD X');
		$data['chart'] = $this->Sepuluh_Besar_Anak_Dewasa_Model->GetDataSepuluh_Besar_Anak_DewasaForChart();
		
		//SET SESSION FOR PRINT OUT
		$this->session->set_userdata('report', $data);
        
		for($i=0;$i<sizeof($data['chart']);$i++) {
			$this->chart->addCategory($data['chart'][$i]['code']);
		}
		$this->chart->addDataset('Anak');
		for($i=0;$i<sizeof($data['chart']);$i++) {
			$this->chart->addChartData($data['chart'][$i]['anak']);
		}
		
		$this->chart->addDataset('Dewasa');
		for($i=0;$i<sizeof($data['chart']);$i++) {
			$this->chart->addChartData($data['chart'][$i]['dewasa']);
		}
        /*
		for($i=0;$i<sizeof($data['chart']);$i++) {
		...
		/*CHART END*/
        $this->load->view('report/sepuluh_besar_anak_dewasa_list', $data);
	}
	...
	function printout($title="") {
		...
		$this->load->view('report/sepuluh_besar_anak_dewasa_print', $data);
	...
	function _view($data = array()) {
		...
		$this->load->view('report/sepuluh_besar_anak_dewasa', $data);
