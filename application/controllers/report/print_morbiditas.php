<?php
Class Print_Morbiditas extends Controller {
	var $title = '';
	var $report_title = 'Morbiditas ';

	function __construct() {
		parent::Controller();
		if(!$this->session->userdata('id')) redirect('login');
		$this->load->model('report/Print_Morbiditas_Model');
	...
	function index($data="") {
		...
		$data['profile'] = $this->Xprofile_Model->GetProfile();
       	$data['combo_payment_type'] = $this->Print_Morbiditas_Model->GetComboPaymentType();
		$this->_view($data);	      	
	}
	...
	function process_form() {
		...
		if($this->input->post('payment_type_id') != '') {
			$data['report_title'] .= ' Jenis Pasien ' . $this->Print_Morbiditas_Model->GetDataPaymentTypeName($this->input->post('payment_type_id'));
		}
		...
		$data['list'] = $this->Print_Morbiditas_Model->GetDataVisitSensus();
        $this->load->view('report/print_morbiditas_list', $data);
	}
    ...
    function excel($title="") {
		...
		if($report_param['payment_type_id'] != '') {
			$data['report_title'] .= ' Jenis Pasien ' . $this->Print_Morbiditas_Model->GetDataPaymentTypeName($report_param['payment_type_id']);
		}
		
		
        header('Content-Type: application/vnd.pwg-xhtml-print+xml');
        header('Content-Disposition: inline; filename=Morbiditas_' . underscore($data["report_sub_title"]) . '.xls');
        header('Expires: 0');
        header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
        header('Pragma: public');
		//print_r($report_param);
		$data['list'] = $this->Print_Morbiditas_Model->GetDataVisitSensusPrint($report_param);
		//$this->load->view('_header_print_full_landscape', $data);
		$this->load->view('report/print_morbiditas_excel', $data);
	...
	function printout($title="") {
		...
		$data['list'] = $this->Print_Morbiditas_Model->GetDataVisitSensusPrint($report_param);
		$this->load->view('_header_print_full_landscape', $data);
		$this->load->view('report/print_morbiditas_cetak', $data);
		$this->load->view('_footer_print_full');
	}
	...
	function _view($data = array()) {
		...
		$this->load->view('report/print_morbiditas', $data);
