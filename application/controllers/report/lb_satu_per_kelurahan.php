...
Class LB_Satu extends Controller {
	var $title = '';
	...
	function __construct() {
		...
		$this->load->model('report/LB_Satu_Model');     
		//$this->load->model('report/LB_Satu_Model');
	...
	function index($data="") {
        ...
        $data['combo_district'] = $this->LB_Satu_Model->GetComboDistrict();
        $data['combo_kelompok_penyakit'] = $this->LB_Satu_Model->GetComboKelompokPenyakit();
	...
	function process_form() {
		...
		$data['report_title'] = $this->LB_Satu_Model->GetNamaKelompokPenyakit($this->input->post('icd_group_id'));
		$data['report_sub_title_wilayah'] = "";
		if($this->input->post('general_spesifik') == 'spesifik') {
			$data['report_sub_title_wilayah'] = $this->LB_Satu_Model->GetRegionName($this->input->post('village_id'), $this->input->post('sub_district_id'), $this->input->post('district_id'));
		} else {
			switch($region) {
				case "in" :
		...
		$data['temp'] = $this->LB_Satu_Model->GetDataLB_SatuForChart();
		$data['jenis_penyakit'] = $this->LB_Satu_Model->GetJenisPenyakit();
		//echo "<pre>";
        ...
        $this->load->view('report/lb_satu_list', $data);
	}
	...
	function printout($title="") {
		...
		$data['report_title'] = $this->LB_Satu_Model->GetNamaKelompokPenyakit($arr_post['icd_group_id']);
		//$data['report_title'] = $this->report_title;
		$data['report_sub_title_wilayah'] = "";
		if($arr_post['general_spesifik'] == 'spesifik') {
			$data['report_sub_title_wilayah'] = $this->LB_Satu_Model->GetRegionName($arr_post['village_id'], $arr_post['sub_district_id'], $arr_post['district_id']);
		} else {
		...
		$data['temp'] = $this->LB_Satu_Model->GetDataLB_SatuForPrint();
		$data['jenis_penyakit'] = $this->LB_Satu_Model->GetJenisPenyakitForPrint();
		//echo "<pre>";
		for($i=0;$i<sizeof($data['jenis_penyakit']);$i++) {
		...
		$this->load->view('report/lb_satu_print', $data);
	...
	function _view($data = array()) {
		...
		$this->load->view('report/lb_satu', $data);
	...
	function get_region_by_village_id($villageId) {
		//kabupaten
        $district = $this->LB_Satu_Model->GetDistrictByVillageId($villageId);
		$ret['district_id'] = $district['id'];

        $sub_district = $this->LB_Satu_Model->GetSubDistrictByVillageId($villageId);
        $arr_sub_district = $this->LB_Satu_Model->GetComboSubDistrictByVillageId($villageId);
        ...
        $arr_village = $this->LB_Satu_Model->GetComboVillageById($villageId);
	...
	function get_sub_district($districtId='1') {
        $data = $this->LB_Satu_Model->GetComboSubDistrict($districtId);
	...
	function get_village($subDistrictId='1') {
        $data = $this->LB_Satu_Model->GetComboVillage($subDistrictId);
		$ret = '<option value="">--- '.$this->lang->line('form_all').' ---</option>';
		for($i=0;$i<sizeof($data);$i++) {
			$ret .= '<option value="'.$data[$i]['id'].'">'.$data[$i]['name'].'</option>';
		}
