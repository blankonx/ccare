...
Class Visit_Sensus extends Controller {
	...
	function index($data="") {
		...
		$data['profile'] = $this->Xprofile_Model->GetProfile();
       	$data['combo_payment_type'] = $this->Visit_Sensus_Model->GetComboPaymentType();
		$this->_view($data);	      	
	}
	...
	function process_form() {
		...
		if($this->input->post('payment_type_id') != '') {
			$data['report_title'] .= ' Jenis Pasien ' . $this->Visit_Sensus_Model->GetDataPaymentTypeName($this->input->post('payment_type_id'));
		}
    ...
    function excel($title="") {
		...
		if($report_param['payment_type_id'] != '') {
			$data['report_title'] .= ' Jenis Pasien ' . $this->Visit_Sensus_Model->GetDataPaymentTypeName($report_param['payment_type_id']);
		}
		...
		$this->load->view('report/visit_sensus_excel', $data);
