...
Class Queue_List extends Controller {
    ...
    function __construct() {
        ...
        $this->load->model('visit/Queue_List_Model');
    }
    
    function index() {
        return $this->result($this->input->post('search_clinic_id') . '_' . $this->input->post('search_name') . '_' . $this->input->post('search_show_served') . '_' . str_replace("/", "-", $this->input->post('visit_date_start')) . '_' . str_replace("/", "-", $this->input->post('visit_date_end')) . "/" . $this->input->post('xcurrent_page'));
    }
    ...
    function result($str_search='', $start=0) {
		...
		//print_r($data);
		$paging = array();
		$paging['base_url'] = base_url() . 'visit/queue_list/result/' . $str_search . '/';
...
//VIEW//
    function _view($data = array()) {
        $this->load->view('visit/queue_list', $data);
    }
}
