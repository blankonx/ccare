...
Class Backup_Database_List extends Controller {
    ...
    function __construct() {
        parent::Controller();
        //if(!$this->session->backup_databasedata('id')) redirect('login');
        $this->load->model('tools/Backup_Database_List_Model');
    }
    ...
    function result($str_search='', $start=0) {
		...
		$data['list'] = $this->Backup_Database_List_Model->GetData($str_search, $start, $this->offset);
		$data['total'] = $this->Backup_Database_List_Model->getCount();
		$data['start'] = $start;

		//print_r($data);
		$paging = array();
		$paging['base_url'] = base_url() . 'tools/backup_database_list/result/' . $str_search . '/';
...
//VIEW//
    function _view($data = array()) {
        $this->load->view('tools/backup_database_list', $data);
    }
}
