...
Class Empty_Database_List extends Controller {
    ...
    function __construct() {
        parent::Controller();
        //if(!$this->session->empty_databasedata('id')) redirect('login');
        $this->load->model('tools/Empty_Database_List_Model');
    }
    ...
    function result($str_search='', $start=0) {
		...
		$data['list'] = $this->Empty_Database_List_Model->GetData($str_search, $start, $this->offset);
		$data['total'] = $this->Empty_Database_List_Model->getCount();
		$data['start'] = $start;

		//print_r($data);
		$paging = array();
		$paging['base_url'] = base_url() . 'tools/empty_database_list/result/' . $str_search . '/';
...
//VIEW//
    function _view($data = array()) {
        $this->load->view('tools/empty_database_list', $data);
    }
}
