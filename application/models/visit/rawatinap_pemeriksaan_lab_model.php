<?php
Class Rawatinap_Pemeriksaan_Lab_Model extends Model {
...
//GET//
	function GetList($visitInpatientId, $limit, $offset) {
        $sql = $this->db->query("
            SELECT 
                SQL_CALC_FOUND_ROWS 
                pl.id as id,
    ...
    function DoAddDataPemeriksaanLab() {
        ...
        $sql_visit = $this->db->query("
            ...
            FROM
                visits v 
				JOIN visits_inpatient vi ON (vi.visit_id = v.id)
            WHERE
                vi.id=?
        ", 
            array(
				$this->session->userdata('id'),
                $this->input->post('pl_visit_inpatient_id')
            )
        );
        ...
        $item = $_POST['rawatinap_pemeriksaan_lab_item'];
        $arr_item = array();
